...
Original file is located at
    https://colab.research.google.com/drive/15qL9RhSVyN6bCh1AilRZockjVanxeee8
"""
...
class ResNet34Modified(nn.Module):
    def __init__(self, num_classes=5, use_se=True):
        ...
        self.stage1 = self._make_stage(64, 64, 3, stride=1, use_se=use_se)
        self.stage2 = self._make_stage(64, 128, 4, stride=2, use_se=use_se)
        self.stage3 = self._make_stage(128, 256, 6, stride=2, use_se=use_se)
        self.stage4 = self._make_stage(256, 512, 3, stride=2, use_se=use_se)
    ...
    def _make_stage(self, in_channels, out_channels, num_blocks, stride, use_se=True):
        downsample = None
        if stride != 1 or in_channels != out_channels:
            downsample = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=stride, bias=False)
        layers = [PreActResNetBlock(in_channels, out_channels, stride, downsample, use_se)]
        for _ in range(1, num_blocks):
            layers.append(PreActResNetBlock(out_channels, out_channels, use_se=use_se))
...
print("\n" + "="*60)
print("MENGECEK ARSITEKTUR RESNET-34 MODIFIKASI (Pre-act + SE)")
print("="*60)
...
print("\n🚀 MULAI TRAINING RESNET-34 MODIFIKASI (Pre-act + SE) - 5 FOLD")
...
for fold in range(5):
    print(f"\n{'='*60}")
    print(f" FOLD {fold+1}/5 — RESNET-34 MODIFIKASI (Pre-act + SE)")
    print('='*60)
    ...
    for epoch in range(EPOCHS):
        ...
        if val_f1 > best_val_f1:
            best_val_f1 = val_f1
            counter = 0
            torch.save(model.state_dict(), f'resnet34_mod_fold{fold}_best.pth')
        else:
            counter += 1
            if counter >= patience:
...
print(f"\n FOLD TERBAIK: Fold {best_fold} dengan F1 = {best_f1:.4f}")
torch.save(torch.load(f'resnet34_mod_fold{best_fold}_best.pth'), 'resnet34_mod_best.pth')
print(" Model ResNet-34 modifikasi terbaik disimpan sebagai 'resnet34_mod_best.pth'")

"""FOLD TERBAIK: Fold 3 dengan F1 = 0.9073
 Model ResNet-34 modifikasi terbaik disimpan sebagai 'resnet34_mod_best.pth'

"""

model = get_model().to(device)
model.load_state_dict(torch.load('resnet34_mod_best.pth'))
model.eval()
...
print("\n === METRIK PER-KELAS (RESNET-34 MODIFIKASI) ===")
...
plt.title('Confusion Matrix - ResNet-34 (Pre-act + SE)')
plt.colorbar()
...
plt.tight_layout()
plt.savefig('confusion_matrix_resnet34_mod.png', dpi=150)
plt.show()

print("✅ Confusion matrix disimpan ke 'confusion_matrix_resnet34_mod.png'")

""" METRIK PER-KELAS (RESNET-34 MODIFIKASI) ===
              precision    recall  f1-score   support

 nasi_goreng     0.9783    0.9783    0.9783        46

     rendang     0.8824    1.0000    0.9375        45

   soto_ayam     0.9024    0.8605    0.8810        43

       bakso     0.9714    0.7727    0.8608        44

   gado_gado     0.8333    0.9302    0.8791        43

    accuracy                         0.9095       221


   macro avg     0.9136    0.9083    0.9073       221


weighted avg     0.9144    0.9095    0.9083       221
