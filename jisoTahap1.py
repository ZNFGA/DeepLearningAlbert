...
Original file is located at
    https://colab.research.google.com/drive/10UL6pDuinu_ziKrPHqgWd5de6z5wwjoC
"""
...
import pickle
with open('fold_splits_plain34.pkl', 'wb') as f:
...
class PlainBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, downsample=None):
        ...
        self.downsample = downsample  # Digunakan hanya untuk matching dimensi

    def forward(self, x):
        identity = x  # Disimpan untuk konsistensi dengan ResNet, tapi TIDAK DIGUNAKAN
        ...
        # Ini adalah perbedaan utama dengan ResNet!

        # Jika dimensi berubah, kita tetap lakukan downsample pada identity
        # TAPI TIDAK MENAMBAHKANNYA KE OUTPUT → hanya untuk menjaga bentuk tensor
        if self.downsample is not None:
            identity = self.downsample(identity)

        # Langsung ReLU pada output konvolusi
        out = F.relu(out)
        return out
...
def get_model():
    """
    Mengembalikan model Plain-34 yang dilatih dari awal (from scratch).
    Tidak ada pretrained weights.
    """
    return Plain34(num_classes=5)

print("\n" + "="*60)
print("MENGECEK ARSITEKTUR PLAIN-34")
print("="*60)
...
print("\n🚀 MULAI TRAINING PLAIN-34 (FROM SCRATCH) - 5 FOLD")

best_fold = -1
best_f1 = 0
all_fold_results = []

EPOCHS = 50          # Lebih banyak karena from scratch
BATCH_SIZE = 32
LR = 0.01            # Learning rate lebih besar untuk SGD
MOMENTUM = 0.9
WEIGHT_DECAY = 1e-4

for fold in range(5):
    print(f"\n{'='*60}")
    print(f" FOLD {fold+1}/5 — PLAIN-34 (NO RESIDUAL)")
    print('='*60)
    ...
    # Optimizer & Scheduler (sesuai paper ResNet asli)
    ...
    for epoch in range(EPOCHS):
        ...
        if val_f1 > best_val_f1:
            best_val_f1 = val_f1
            counter = 0
            torch.save(model.state_dict(), f'plain34_fold{fold}_best.pth')
        else:
            counter += 1
            if counter >= patience:
...
torch.save(torch.load(f'plain34_fold{best_fold}_best.pth'), 'plain34_best.pth')
print("✅ Model Plain-34 terbaik disimpan sebagai 'plain34_best.pth'")

# --- EVALUASI AKHIR ---
model = get_model().to(device)
model.load_state_dict(torch.load('plain34_best.pth'))
model.eval()
...
"""
🏆 FOLD TERBAIK: Fold 1 dengan F1 = 0.3986
✅ Model Plain-34 terbaik disimpan sebagai 'plain34_best.pth'
"""

print("\n📊 === METRIK PER-KELAS (PLAIN-34) ===")
...
plt.title('Confusion Matrix - Plain-34 (Tanpa Residual)')
plt.colorbar()
...
plt.tight_layout()
plt.savefig('confusion_matrix_plain34.png', dpi=150)
plt.show()

print("✅ Confusion matrix disimpan ke 'confusion_matrix_plain34.png'")

"""
              precision    recall  f1-score   support
            

 nasi_goreng     0.4516    0.2979    0.3590        47

     rendang     0.4255    0.8696    0.5714        46

   soto_ayam     0.4808    0.5952    0.5319        42

       bakso     0.3871    0.2727    0.3200        44

   gado_gado     0.4286    0.1395    0.2105        43

    accuracy                         0.4369       222

   macro avg     0.4347    0.4350    0.3986       222

weighted avg     0.4345    0.4369    0.3992       222
"""
